...
import { completeAt, indexC, type CIndex } from '../src/index.js'
...
function passes(fx: Fixture, index: CIndex): boolean {
  const labels = completeAt(index, fx.source, fx.offset).map((c) => c.label)
  if (fx.mode === 'all') {
...
const indexFor = (fx: Fixture): CIndex => indexC([{ path: fx.name, text: fx.source }])
