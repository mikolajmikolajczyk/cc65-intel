...
import type { CIndex, CSemanticDiagnostic } from './types.js'
import { parseC } from './parse.js'
import { deepChild, slice, walk } from './ast.js'
import { lookupType, resolveChainType } from './resolve.js'
...
function checkMemberAccess(
  ...
  const type = resolveChainType(index, text, m[1] ?? '', m[2] ?? '', field.from)
...
export function diagnoseC(index: CIndex, text: string): CSemanticDiagnostic[] {
  ...
  walk(root, (n) => {
    if (n.name === 'FieldExpression') checkMemberAccess(n, text, index, out)
