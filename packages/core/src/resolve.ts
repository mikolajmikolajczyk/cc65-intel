 ...
 *  (*(struct __vic2*)…)`. */
function baseType(index: CIndex, text: string, base: string, offset: number): string | null {
  return resolveVarType(text, base, offset) ?? index.symbols.get(base)?.type ?? null
}
...
export function resolveChainType(
  ...
  const bt = baseType(index, text, base, offset)
