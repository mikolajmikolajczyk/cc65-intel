import type { CLocation, SourceFile } from './types.js'
import { parseC } from './parse.js'
import { slice, walk, wordAt } from './ast.js'
...
export function findReferences(files: SourceFile[], name: string): CLocation[] {
  const out: CLocation[] = []
  for (const f of files) {
    const root = parseC(f.text).topNode
    walk(root, (n) => {
...
export function referencesAt(files: SourceFile[], text: string, offset: number): CLocation[] {
  const word = wordAt(text, offset)
  return word ? findReferences(files, word.word) : []
}
